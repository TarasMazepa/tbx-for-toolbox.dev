...
var subscriptions_manage = /walmart.com\/subscriptions\/manage/,window2 = window;
...
var newDate = (x) => new Date(x),querySelectorAll = (x,q) => x.querySelectorAll(q),
...
cards.forEach(card => {
var name = '',nameEl = querySelector(card,'.ld_AY');
if (nameEl) name = nameEl.innerText.trim();

if (!name) {
var aEl = querySelector(card,'a[aria-label]');
if (aEl) name = aEl.getAttribute('aria-label');
}

if (!name) {
var imgAlt = querySelector(card,'img');
if (imgAlt) name = imgAlt.alt;
}

if (!name) return;

var imgEl = querySelector(card,'img'),imgSrc = imgEl ? imgEl.src :'';
if (imgSrc) imgSrc = imgSrc.split('?')[0] + '?odnHeight=80&odnWidth=80&odnBg=FFFFFF';

var dateStr = '',bEls = querySelectorAll(card,'b');
bEls.forEach(b => {
...
if (!dateStr) {
var spans = querySelectorAll(card,'span');
...
var freqStr = '',spansFreq = querySelectorAll(card,'span');
...
var attempts = 0,mode = 'list',// 'list' (find order ID) -> 'detail' (scrape items)
...
if (mode === 'list') {
var h2s = querySelectorAll(doc,'h2');
...
if (orderContainer) {
var detailBtn = querySelector(orderContainer,'button[data-automation-id^="view-order-details-link-"],a[href^="/orders/"]');
if (detailBtn) {
...
var itemCards = querySelectorAll(doc,'div[data-testid="itemtile-stack"]');
if (itemCards.length > 0) {
clearInterval(checkInterval);

var processingItems = [];
itemCards.forEach(card => {
var nameEl = querySelector(card,'[data-testid="productName"]');
if (nameEl) {
...
items.forEach(item => {
var cleanSubName = item.n.toLowerCase().trim(),
...
items.forEach(item => {
var startingDateStr = item.p ? item.p :item.date;
item.d = newDate(`${startingDateStr} ${currentYear}`);

var mBase = months[item.d.getMonth()],dBase = item.d.getDate().toString().padStart(2,'0');
...
if (match) {
var amount = parseInt(match[1]) || 1,unit = match[2].toLowerCase();
item.fd = (unit === 'week') ? amount * 7 :amount * 30;

var fwdDate = newDate(item.d.getTime());
for (var i = 0;i < 15;i++) {
if (unit === 'week') fwdDate.setDate(fwdDate.getDate() + (amount * 7));else if (unit === 'month') fwdDate.setMonth(fwdDate.getMonth() + amount);

var m = months[fwdDate.getMonth()],d = fwdDate.getDate().toString().padStart(2,'0');
...
var allExtrapolatedDates = items.flatMap(i => i.allDates),uniqueDates = [...new Set(allExtrapolatedDates)];
uniqueDates.sort((a,b) => newDate(a) - newDate(b));
uniqueDates = uniqueDates.filter(d => newDate(d) >= today);

var maxCols = 5,colDates = uniqueDates.slice(0,maxCols),maxDateObj = newDate(colDates[colDates.length - 1]);

items.forEach(item => {
item.nam = '';
for (var d of item.ad) {
...
var freqDisplay = item.freq ? `<span class='freq-subtext'>${item.freq}</span>` :'';
